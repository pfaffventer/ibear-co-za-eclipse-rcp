...
public class ProductCategoryTransaction {
	private SqliteJdbc ibearDb = null;
	private ProductCategoryModel unit = null;
	public ProductCategoryTransaction(ProductCategoryModel unit) throws Exception {
		this.unit = unit;
		try {
			if(unit==null) {
...
//		ibearDb.update("COMMIT");
	...
	public void insert(ProductCategoryModel newProductCategory) throws Exception {
		ResultSet rs = ibearDb.result("SELECT MAX(UnitSequence) [UnitSequence] from ProductCategory");
		if(!rs.next()) {
			rs.close();
		}
		...
		rs.close();
		next = next+1;
		ibearDb.update(newProductCategory.toInsertSql(next));
	}
...
//			rsCheckLock.close();
	...
	public ProductCategoryModel getUnit() {
		return unit;
	}
	public void setUnit(ProductCategoryModel unit) {
		this.unit = unit;
	}
