...
public class PurchaseOrderTransaction {
	...
	private PurchaseOrderModel unit = null;
	public PurchaseOrderTransaction(PurchaseOrderModel unit) throws Exception {
		this.unit = unit;
		try {
			if(unit==null) {
...
//		ibearDb.update("COMMIT");
	...
	public void insert(PurchaseOrderModel newPurchaseOrder) throws Exception {
		ResultSet rs = ibearDb.result("SELECT MAX(UnitSequence) [UnitSequence] from PurchaseOrder");
		if(!rs.next()) {
			rs.close();
		}
		...
		rs.close();
		next = next+1;
		ibearDb.update(newPurchaseOrder.toInsertSql(next));
	}
...
//			rsCheckLock.close();
	...
	public PurchaseOrderModel getUnit() {
		return unit;
	}
	public void setUnit(PurchaseOrderModel unit) {
		this.unit = unit;
	}
