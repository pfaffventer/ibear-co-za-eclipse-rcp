...
public class PurchaseOrderLink extends PropertyModel {
	
	private SqliteJdbc databasePurchaseOrderLink = null;
	
	private List<PurchaseOrderModel> data = new ArrayList<PurchaseOrderModel>();
	private List<PurchaseOrderModel> filterData = new ArrayList<PurchaseOrderModel>();
	...
	protected JFilter<PurchaseOrderModel> filter;
	
	public PurchaseOrderLink() throws Exception {
		try {
			databasePurchaseOrderLink = new SqliteJdbc(DatabaseConstant.DB_CONNECTION);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		} 
	...
	public void disconnect() {
		try {
			databasePurchaseOrderLink.disconnect();
		} catch (Exception e) {
		}
	}
	public List<PurchaseOrderModel> pop() throws Exception {
		String query = "select * from PurchaseOrder order by PurchaseOrder";
		QueryRunner qr = new QueryRunner();
		setData((List<PurchaseOrderModel>) qr.query(databasePurchaseOrderLink.getConnection(),query,new BeanListHandler<PurchaseOrderModel>(PurchaseOrderModel.class)));
		return getData();
	}
	public List<PurchaseOrderModel> toPurchaseOrderModelList(String where) throws Exception {
		String query = "select * from PurchaseOrder";
		if(where!=null) {
			query = query + where;
		}
		query = query + " order by PurchaseOrder";
		QueryRunner qr = new QueryRunner();
		setData((List<PurchaseOrderModel>) qr.query(databasePurchaseOrderLink.getConnection(),query,new BeanListHandler<PurchaseOrderModel>(PurchaseOrderModel.class)));
		return getData();
	}
	public void stringSearch(String searchString) {
		Class<PurchaseOrderModel> model = PurchaseOrderModel.class;
		Method[] methods = model.getDeclaredMethods();
		filter = new JFilter<PurchaseOrderModel>(data, PurchaseOrderModel.class);
		List<PurchaseOrderModel> tempSet = new ArrayList<PurchaseOrderModel>();
		List<PurchaseOrderModel> resultSet = new ArrayList<PurchaseOrderModel>();
				...
				for (Method method : methods) {
					if(method.getReturnType().getSimpleName().equals("String")) {
						if(method.getName().startsWith("get")) {
							tempSet.addAll(filter.filter("{'" + method.getName() + "':{'$cts':'?1'}}",value.trim()).out(new ArrayList<PurchaseOrderModel>()));
						}
					}
				}
				resultSet = tempSet;
				filter = new JFilter<PurchaseOrderModel>(resultSet, PurchaseOrderModel.class);
				tempSet = new ArrayList<PurchaseOrderModel>();
				filtered=true;
			}
		}
		if(filtered) {
	...
	public void filter(String column,List<String> values) {
		...
		if(values.size()>0) {
			for(String value:values) {
				tempSet.addAll(filter.filter("{'get" + column.trim() + "':{'trim':'?1'}}",value.trim()).out(new ArrayList<PurchaseOrderModel>()));
			}
			resultSet = tempSet;
			filter = new JFilter<PurchaseOrderModel>(resultSet, PurchaseOrderModel.class);
			tempSet = new ArrayList<PurchaseOrderModel>();
	...
	public List<PurchaseOrderModel> toPurchaseOrderModelList(String where,Map<String,List<String>> initFilter) throws Exception {
		String query = "select * from PurchaseOrder order by PurchaseOrder";
		if(where!=null) {
			query = query + where;
		}
		QueryRunner qr = new QueryRunner();
		initFilter(initFilter,(List<PurchaseOrderModel>) qr.query(databasePurchaseOrderLink.getConnection(),query,new BeanListHandler<PurchaseOrderModel>(PurchaseOrderModel.class)));
		return getData();
	}
	public void initFilter(Map<String,List<String>> initFilter,List<PurchaseOrderModel> allData) {
		filter = new JFilter<PurchaseOrderModel>(allData, PurchaseOrderModel.class);
		List<PurchaseOrderModel> tempSet = new ArrayList<PurchaseOrderModel>();
		List<PurchaseOrderModel> resultSet = new ArrayList<PurchaseOrderModel>();
		boolean filtered = false;
		for(Entry<String,List<String>> column:initFilter.entrySet()) {
			...
			if(values.size()>0) {
				for(String value:values) {
					tempSet.addAll(filter.filter("{'get" + columnName.trim() + "':{'trim':'?1'}}",value.trim()).out(new ArrayList<PurchaseOrderModel>()));
				}
				resultSet = tempSet;
				filter = new JFilter<PurchaseOrderModel>(resultSet, PurchaseOrderModel.class);
				tempSet = new ArrayList<PurchaseOrderModel>();
	...
	public Set<String> toDistinctFilterSet(String column) throws Exception {
		filter = new JFilter<PurchaseOrderModel>(filterData, PurchaseOrderModel.class);
		Set<String> itemTree = new TreeSet<String>();
		try {
			itemTree.addAll(filter.filter("{'$not':[{'get" + column + "':{'$sw':'?1'}}]}", "_iBear_").map(column).out(new TreeSet()));
		} catch (Exception e) {
	...
	public void doDelete(PurchaseOrderModel unit,boolean isFiltered) throws Exception {
		filter = new JFilter<PurchaseOrderModel>(data, PurchaseOrderModel.class);
		setData(filter.filter("{'$not':[{'getUnitSequence':{'$eq':'?1'}}]}", unit.getUnitSequence()).out(new ArrayList<PurchaseOrderModel>()));
		if(isFiltered) {
			filter = new JFilter<PurchaseOrderModel>(filterData, PurchaseOrderModel.class);
			setFilterData(filter.filter("{'$not':[{'getUnitSequence':{'$eq':'?1'}}]}", unit.getUnitSequence()).out(new ArrayList<PurchaseOrderModel>()));
		}
	}
	@SuppressWarnings({ "unchecked", "rawtypes" })
	...
	public void doQuery(String column, List<String> queryItems) {
		...
		for(String selected:queryItems) {
			tempSet.addAll(filter.filter("{'get" + column + "':{'trim':'?1'}}",selected.trim()).out(new ArrayList<PurchaseOrderModel>()));
		}
	...
	public void doSelect() {
		...
		for(BCombo bcombo:multiSet) {
			if(bcombo.getSelection().size()>0) {
				for(String selected:bcombo.getSelection()) {
					tempSet.addAll(filter.filter("{'get" + bcombo.getColumnName() + "':{'trim':'?1'}}",selected.trim()).out(new ArrayList<PurchaseOrderModel>()));
				}
				resultSet = tempSet;
				filter = new JFilter<PurchaseOrderModel>(resultSet, PurchaseOrderModel.class);
				tempSet = new ArrayList<PurchaseOrderModel>();
				...
				for(Entry<String, TreeSet<String>> selection:bcombo.getBrowseSelection().entrySet()) {
					...
					if(columnData.size()>0) {
						for(String data:columnData) {
							tempSet.addAll(filter.filter("{'get" + columnName.trim() + "':{'trim':'?1'}}",data.trim()).out(new ArrayList<PurchaseOrderModel>()));
						}
						resultSet = tempSet;
						filter = new JFilter<PurchaseOrderModel>(resultSet, PurchaseOrderModel.class);
						tempSet = new ArrayList<PurchaseOrderModel>();
		...
		for(BCombo bcombo:dateSet) {
			if(bcombo.getDatedDialog().getSelection().size()>0) {
				...
				tempSet.addAll(filter.filter(query,from,to).out(new ArrayList<PurchaseOrderModel>()));
				resultSet = tempSet;
				filter = new JFilter<PurchaseOrderModel>(resultSet, PurchaseOrderModel.class);
				tempSet = new ArrayList<PurchaseOrderModel>();
				filtered=true;
	...
	public List<PurchaseOrderModel> getData() {
		return data;
	}
	public void setData(List<PurchaseOrderModel> data) {
		firePropertyChange("data", this.data, this.data = data);
	}
	public List<PurchaseOrderModel> getFilterData() {
		return filterData;
	}
	public void setFilterData(List<PurchaseOrderModel> filterData) {
	...
	public int nextUnitSequence() throws Exception {
		...
		try {
			...
			ResultSet rsNext = nextConnect.result("select * from UnitSequence where UnitName = 'PurchaseOrder'");
			if(rsNext.next()) {
				next = rsNext.getInt("NextSequence");
			}
			rsNext.close();
			next++;
			nextConnect.update("update UnitSequence set NextSequence = " + next + " where UnitName = 'PurchaseOrder'");
		} catch (Exception e) {
			nextConnect.update("ROLLBACK TO next_purchaseorder_sequence");
			throw e;
		}
