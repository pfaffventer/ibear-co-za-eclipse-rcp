...
public class ProductEdit extends Composite {
	...
	private CTabItem productTab;
	private Composite productComposite;
	private Text search;
	...
	int labelWidth = 200;
	public ProductEdit(Composite parent, int style, int labelWidth) throws Exception {
		super(parent, style);
		...
		productTab = new CTabItem(folder, SWT.NONE);
		productTab.setText("Product");
		productComposite = new Composite(folder, SWT.NONE);
		productTab.setControl(productComposite);
		GridLayout gl_productComposite = new GridLayout(3, false);
		gl_productComposite.verticalSpacing = 3;
		gl_productComposite.marginWidth = 0;
		gl_productComposite.marginHeight = 0;
		gl_productComposite.horizontalSpacing = 3;
		productComposite.setLayout(gl_productComposite);
		search = new Text(productComposite, SWT.SEARCH | SWT.ICON_SEARCH | SWT.CANCEL | SWT.BORDER);
		...
		Button add = new Button(productComposite, SWT.NONE);
		...
		Button delete = new Button(productComposite, SWT.NONE);
		if(meta.IS_READ_ONLY) {
		...
		viewer = new TableViewer(productComposite, SWT.BORDER);
		...
		createEditors(null);
		final ProductEditFilter rowFilter = new ProductEditFilter();
	...
	public void createEditors(ProductModel data) throws Exception {
		if (data != null) {
			this.editModel = data;
		} else {
	...
	public ProductModel getEditModel() {
		return editModel;
	}
	public void setEditModel(ProductModel editModel) {
...
class ProductEditFilter extends ViewerFilter {
...
class ProductEditComparator extends ViewerComparator {
	private int propertyIndex;
	private static final int DESCENDING = 1;
	private int direction = DESCENDING;
	public ProductEditComparator() {
