...
public class PurchaseOrderLineEdit extends Composite {
	...
	private CTabItem purchaseorderlineTab;
	private Composite purchaseorderlineComposite;
	private Text search;
	private TableViewer viewer;
	private Table table;
	private PurchaseOrderLine meta = new PurchaseOrderLine();
	private List<UnitEditor> editorData = null;
	private PurchaseOrderLineModel editModel = null;
	...
	int labelWidth = 200;
	public PurchaseOrderLineEdit(Composite parent, int style, int labelWidth) throws Exception {
		super(parent, style);
		...
		purchaseorderlineTab = new CTabItem(folder, SWT.NONE);
		purchaseorderlineTab.setText("PurchaseOrderLine");
		purchaseorderlineComposite = new Composite(folder, SWT.NONE);
		purchaseorderlineTab.setControl(purchaseorderlineComposite);
		GridLayout gl_purchaseorderlineComposite = new GridLayout(3, false);
		gl_purchaseorderlineComposite.verticalSpacing = 3;
		gl_purchaseorderlineComposite.marginWidth = 0;
		gl_purchaseorderlineComposite.marginHeight = 0;
		gl_purchaseorderlineComposite.horizontalSpacing = 3;
		purchaseorderlineComposite.setLayout(gl_purchaseorderlineComposite);
		search = new Text(purchaseorderlineComposite, SWT.SEARCH | SWT.ICON_SEARCH | SWT.CANCEL | SWT.BORDER);
		...
		Button add = new Button(purchaseorderlineComposite, SWT.NONE);
		...
		Button delete = new Button(purchaseorderlineComposite, SWT.NONE);
		if(meta.IS_READ_ONLY) {
		...
		viewer = new TableViewer(purchaseorderlineComposite, SWT.BORDER);
		...
		createEditors(null);
		final PurchaseOrderLineEditFilter rowFilter = new PurchaseOrderLineEditFilter();
	...
	public void createEditors(PurchaseOrderLineModel data) throws Exception {
		if (data != null) {
			this.editModel = data;
		} else {
	...
	public PurchaseOrderLineModel getEditModel() {
		return editModel;
	}
	public void setEditModel(PurchaseOrderLineModel editModel) {
...
class PurchaseOrderLineEditFilter extends ViewerFilter {
...
class PurchaseOrderLineEditComparator extends ViewerComparator {
	private int propertyIndex;
	private static final int DESCENDING = 1;
	private int direction = DESCENDING;
	public PurchaseOrderLineEditComparator() {
