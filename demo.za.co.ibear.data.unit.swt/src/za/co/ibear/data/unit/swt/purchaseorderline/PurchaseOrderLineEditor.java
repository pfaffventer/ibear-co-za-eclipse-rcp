...
import za.co.ibear.data.unit.purchaseorderline.PurchaseOrderLineLink;
import za.co.ibear.data.unit.purchaseorderline.PurchaseOrderLineModel;
import za.co.ibear.data.unit.purchaseorderline.PurchaseOrderLineTransaction;
...
public class PurchaseOrderLineEditor extends UnitEditComposite implements IProgressMonitor {
	...
	private PurchaseOrderLineModel selected;
	private Composite filterComposite;
	private TableViewer filterViewer;
	private Table filterTable;
	private Composite editorComposite;
	private ProgressBar progress;
	private PurchaseOrderLineLink data = new PurchaseOrderLineLink();
	...
	private PurchaseOrderLineEdit purchaseorderlineEdit;
	private PurchaseOrderLine meta = new PurchaseOrderLine();
	protected boolean SHELL_PARENT = false;
	private Set<PurchaseOrderLineTransaction> connectionSet = new HashSet<PurchaseOrderLineTransaction>();
	...
	private Text search;
	private PurchaseOrderLineComparator comparator = null;
	private String queryColumn = null;
	private List<String> queryItems = null;
	
	public PurchaseOrderLineEditor(final Composite parent, int style, int[] editorWeights,int labelWidth) throws Exception  {
		super(parent, style);
		this.parent = parent;
		...
		data = new PurchaseOrderLineLink();
		if(!meta.IS_ELEMENT) {
			data.toPurchaseOrderLineModelList(null);
		}
		...
		dataViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				setSelected((PurchaseOrderLineModel) selection.getFirstElement());
			}
		});
		...
		dataViewer.setLabelProvider(new PurchaseOrderLineLabelProvider());
		new TableColumn(dataTable, SWT.LEFT).setText("Product");
		new TableColumn(dataTable, SWT.LEFT).setText("Description");
		new TableColumn(dataTable, SWT.RIGHT).setText("Volume");
		new TableColumn(dataTable, SWT.RIGHT).setText("Price");
		new TableColumn(dataTable, SWT.RIGHT).setText("Total");
		
		for (int i = 0, n = dataTable.getColumnCount(); i < n; i++) {
			dataTable.getColumn(i).pack();
		}
		comparator = new PurchaseOrderLineComparator();
		...
		dataTable.getColumn(2).setWidth(70);
		dataTable.getColumn(3).setWidth(70);
		dataTable.getColumn(4).setWidth(70);
		dataTable.setLinesVisible(true);
		dataTable.setHeaderVisible(true);
		dataViewer.setInput(new WritableList(data.getData(), PurchaseOrderLineModel.class));
		editorComposite = new Composite(sash, SWT.NONE);
		editorComposite.setLayout(new FillLayout(SWT.HORIZONTAL));
		purchaseorderlineEdit = new PurchaseOrderLineEdit(editorComposite, SWT.NONE, labelWidth);
		purchaseorderlineEdit.addPropertyChangeListener("do-edit",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				...
				if(e.getNewValue().toString().equals("UNIT-SELECTED")) {
					setSelected((PurchaseOrderLineModel) dataTable.getSelection()[0].getData());
				}
				dataTable.setRedraw(true);
				dataViewer.refresh();
			}
		});
		purchaseorderlineEdit.addPropertyChangeListener("add-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				addRecord();
			}
		});
		purchaseorderlineEdit.addPropertyChangeListener("delete-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				deleteRecord();
			}
		});
		GridLayout gridLayout = (GridLayout) purchaseorderlineEdit.getLayout();
			...
			protected IStatus run(final IProgressMonitor monitor) {
				...
				Display.getDefault().syncExec(new Runnable() {
					...
					public void run() {
						...
						monitor.worked(3);
						dataViewer.setInput(new WritableList(data.getFilterData(), PurchaseOrderLineModel.class));
						monitor.worked(4);
					}
				});
				monitor.worked(5);
...
//						prime();
	...
	public void selectDetail(int headerSequence) {
		...
		try {
			data.toPurchaseOrderLineModelList(" WHERE UnitSequence = " + headerSequence);
		} catch (Exception e) {
		e.printStackTrace();
		}
		dataViewer.setInput(new WritableList(data.getData(), PurchaseOrderLineModel.class));
...
//						prime();
	...
	public PurchaseOrderLineModel getSelected() {
		return selected;
	}
	private void deleteRecord() {
		...
		for(Object object:selection.toArray()) {
			PurchaseOrderLineModel purchaseorderlineModel = (PurchaseOrderLineModel) object;
			if(connectionSet.size()==0) {
				try {
					connectionSet.add(new PurchaseOrderLineTransaction(purchaseorderlineModel));
			...
			if(selection.size()>0) {
				try {
					data.doDeleteElement((PurchaseOrderLineModel) selection.getFirstElement(), filtered);
				} catch (Exception e) {
					MessageDialog.openError(parent.getShell(), "Edit Error",e.getMessage());
					return;
				}
			...
			if(selection.size()==0) {
				if(dataTable.getItemCount()>0) {
					...
					purchaseorderlineEdit.destroyEditors();
					try {
						purchaseorderlineEdit.createEditors(new PurchaseOrderLineModel(true));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		try {
			...
			if(meta.IS_ELEMENT) {
				...
				for(Object selected:addSelection) {
					PurchaseOrderLineModel newPurchaseOrderLine = new PurchaseOrderLineModel();
					newPurchaseOrderLine.setUnitSequence(getHeaderSequence());
			newPurchaseOrderLine.setUnitSequence(getHeaderSequence());
			int next = data.nextElementSequence();
			newPurchaseOrderLine.setElementSequence(next);
					Method getProperty;
					Method setProperty;
					for(String columnName:meta.ELEMENT_SELECTOR.getReturnSet()) {
						...
						setProperty = newPurchaseOrderLine.getClass().getDeclaredMethod("set" + columnName,String.class);
						setProperty.setAccessible(true);
						setProperty.invoke(newPurchaseOrderLine, getProperty.invoke(selected));
					}
					if(connectionSet.size()==0) {
						connectionSet.add(new PurchaseOrderLineTransaction(newPurchaseOrderLine));
						connectionSet.iterator().next().begin();
					}
					connectionSet.iterator().next().insert(newPurchaseOrderLine);
					boolean filtered = false;
					if(data.getFilterData().size()!=0) {
						if(data.getFilterData().size()!=data.getData().size()) {
							filtered = true;
					...
					data.getData().add(newPurchaseOrderLine);
					dataViewer.setInput(null);
					if(filtered) {
						data.getFilterData().add(newPurchaseOrderLine);
						dataViewer.setInput(new WritableList(data.getFilterData(), PurchaseOrderLineModel.class));
					} else {
						dataViewer.setInput(new WritableList(data.getData(), PurchaseOrderLineModel.class));
					}
	...
	public void setSelected(PurchaseOrderLineModel selected) {
		boolean error = false;
		if(selected==null) {
			return;
		}
		...
		try {
			if(!error) {
				purchaseorderlineEdit.createEditors(selected);
			}
		} catch (Exception e) {
	...
	public void switchTransaction(PurchaseOrderLineModel unit) throws Exception {
		if(connectionSet.size()>0) {
			connectionSet.iterator().next().commit();
			connectionSet.iterator().next().disconnect();
			connectionSet.remove(connectionSet.iterator().next());
		}
		connectionSet.add(new PurchaseOrderLineTransaction(unit));
		try {
			connectionSet.iterator().next().begin();
		} catch (Exception e) {
			connectionSet.iterator().next().disconnect();
