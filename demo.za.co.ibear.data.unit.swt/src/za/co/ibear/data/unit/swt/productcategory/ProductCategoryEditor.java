...
import za.co.ibear.data.unit.productcategory.ProductCategoryLink;
import za.co.ibear.data.unit.productcategory.ProductCategoryModel;
import za.co.ibear.data.unit.productcategory.ProductCategoryTransaction;
...
public class ProductCategoryEditor extends UnitEditComposite implements IProgressMonitor {
	...
	private ProductCategoryModel selected;
	private Composite filterComposite;
	private TableViewer filterViewer;
	private Table filterTable;
	private Composite editorComposite;
	private ProgressBar progress;
	private ProductCategoryLink data = new ProductCategoryLink();
	...
	private ProductCategoryEdit productcategoryEdit;
	private ProductCategory meta = new ProductCategory();
	protected boolean SHELL_PARENT = false;
	private Set<ProductCategoryTransaction> connectionSet = new HashSet<ProductCategoryTransaction>();
	...
	private Text search;
	private ProductCategoryComparator comparator = null;
	private String queryColumn = null;
	private List<String> queryItems = null;
	
	public ProductCategoryEditor(final Composite parent, int style, int[] editorWeights,int labelWidth) throws Exception  {
		super(parent, style);
		this.parent = parent;
		...
		data = new ProductCategoryLink();
		if(!meta.IS_ELEMENT) {
			data.toProductCategoryModelList(null);
		}
		...
		dataViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				setSelected((ProductCategoryModel) selection.getFirstElement());
			}
		});
		...
		dataViewer.setInput(new WritableList(data.getData(), ProductCategoryModel.class));
		editorComposite = new Composite(sash, SWT.NONE);
		editorComposite.setLayout(new FillLayout(SWT.HORIZONTAL));
		productcategoryEdit = new ProductCategoryEdit(editorComposite, SWT.NONE, labelWidth);
		productcategoryEdit.addPropertyChangeListener("do-edit",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				...
				if(e.getNewValue().toString().equals("UNIT-SELECTED")) {
					setSelected((ProductCategoryModel) dataTable.getSelection()[0].getData());
				}
				dataTable.setRedraw(true);
				dataViewer.refresh();
			}
		});
		productcategoryEdit.addPropertyChangeListener("add-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				addRecord();
			}
		});
		productcategoryEdit.addPropertyChangeListener("delete-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				deleteRecord();
			}
		});
		GridLayout gridLayout = (GridLayout) productcategoryEdit.getLayout();
			...
			protected IStatus run(final IProgressMonitor monitor) {
				...
				Display.getDefault().syncExec(new Runnable() {
					...
					public void run() {
						...
						monitor.worked(3);
						dataViewer.setInput(new WritableList(data.getFilterData(), ProductCategoryModel.class));
						monitor.worked(4);
					}
				});
				monitor.worked(5);
...
//						prime();
	...
	public void selectDetail(int headerSequence) {
		...
		try {
			data.toProductCategoryModelList(" WHERE UnitSequence = " + headerSequence);
		} catch (Exception e) {
		e.printStackTrace();
		}
		dataViewer.setInput(new WritableList(data.getData(), ProductCategoryModel.class));
...
//						prime();
	...
	public ProductCategoryModel getSelected() {
		return selected;
	}
	private void deleteRecord() {
		...
		for(Object object:selection.toArray()) {
			ProductCategoryModel productcategoryModel = (ProductCategoryModel) object;
			if(connectionSet.size()==0) {
				try {
					connectionSet.add(new ProductCategoryTransaction(productcategoryModel));
			...
			if(selection.size()>0) {
				try {
					data.doDelete((ProductCategoryModel) selection.getFirstElement(), filtered);
				} catch (Exception e) {
					MessageDialog.openError(parent.getShell(), "Edit Error",e.getMessage());
					return;
				}
			...
			if(selection.size()==0) {
				if(dataTable.getItemCount()>0) {
					...
					productcategoryEdit.destroyEditors();
					try {
						productcategoryEdit.createEditors(new ProductCategoryModel(true));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		try {
			...
			if(meta.IS_ELEMENT) {
				...
				for(Object selected:addSelection) {
					ProductCategoryModel newProductCategory = new ProductCategoryModel();
					newProductCategory.setUnitSequence(getHeaderSequence());
			int next = data.nextUnitSequence();
			newProductCategory.setUnitSequence(next);
					Method getProperty;
					Method setProperty;
					for(String columnName:meta.ELEMENT_SELECTOR.getReturnSet()) {
						...
						setProperty = newProductCategory.getClass().getDeclaredMethod("set" + columnName,String.class);
						setProperty.setAccessible(true);
						setProperty.invoke(newProductCategory, getProperty.invoke(selected));
					}
					if(connectionSet.size()==0) {
						connectionSet.add(new ProductCategoryTransaction(newProductCategory));
						connectionSet.iterator().next().begin();
					}
					connectionSet.iterator().next().insert(newProductCategory);
					boolean filtered = false;
					if(data.getFilterData().size()!=0) {
						if(data.getFilterData().size()!=data.getData().size()) {
							filtered = true;
					...
					data.getData().add(newProductCategory);
					dataViewer.setInput(null);
					if(filtered) {
						data.getFilterData().add(newProductCategory);
						dataViewer.setInput(new WritableList(data.getFilterData(), ProductCategoryModel.class));
					} else {
						dataViewer.setInput(new WritableList(data.getData(), ProductCategoryModel.class));
					}
	...
	public void setSelected(ProductCategoryModel selected) {
		boolean error = false;
		if(selected==null) {
			return;
		}
		...
		try {
			if(!error) {
				productcategoryEdit.createEditors(selected);
			}
		} catch (Exception e) {
	...
	public void switchTransaction(ProductCategoryModel unit) throws Exception {
		if(connectionSet.size()>0) {
			connectionSet.iterator().next().commit();
			connectionSet.iterator().next().disconnect();
			connectionSet.remove(connectionSet.iterator().next());
		}
		connectionSet.add(new ProductCategoryTransaction(unit));
		try {
			connectionSet.iterator().next().begin();
		} catch (Exception e) {
			connectionSet.iterator().next().disconnect();
