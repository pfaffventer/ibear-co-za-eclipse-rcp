...
import za.co.ibear.data.unit.purchaseorder.PurchaseOrderLink;
import za.co.ibear.data.unit.purchaseorder.PurchaseOrderModel;
import za.co.ibear.data.unit.purchaseorder.PurchaseOrderTransaction;
...
public class PurchaseOrderEditor extends UnitEditComposite implements IProgressMonitor {
	...
	private PurchaseOrderModel selected;
	private Composite filterComposite;
	private TableViewer filterViewer;
	private Table filterTable;
	private Composite editorComposite;
	private ProgressBar progress;
	private PurchaseOrderLink data = new PurchaseOrderLink();
	...
	private PurchaseOrderEdit purchaseorderEdit;
	private PurchaseOrder meta = new PurchaseOrder();
	protected boolean SHELL_PARENT = false;
	private Set<PurchaseOrderTransaction> connectionSet = new HashSet<PurchaseOrderTransaction>();
	...
	private Text search;
	private PurchaseOrderComparator comparator = null;
	private String queryColumn = null;
	private List<String> queryItems = null;
	
	public PurchaseOrderEditor(final Composite parent, int style, int[] editorWeights,int labelWidth) throws Exception  {
		super(parent, style);
		this.parent = parent;
		...
		data = new PurchaseOrderLink();
		if(!meta.IS_ELEMENT) {
			data.toPurchaseOrderModelList(null);
		}
		...
		dataViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				setSelected((PurchaseOrderModel) selection.getFirstElement());
			}
		});
		...
		dataViewer.setLabelProvider(new PurchaseOrderLabelProvider());
		new TableColumn(dataTable, SWT.LEFT).setText("PurchaseOrder");
		...
		comparator = new PurchaseOrderComparator();
		...
		dataTable.getColumn(1).setWidth(120);
		dataTable.getColumn(2).setWidth(270);
		dataTable.getColumn(3).setWidth(70);
		dataTable.getColumn(4).setWidth(210);
		dataTable.setLinesVisible(true);
		dataTable.setHeaderVisible(true);
		dataViewer.setInput(new WritableList(data.getData(), PurchaseOrderModel.class));
		editorComposite = new Composite(sash, SWT.NONE);
		editorComposite.setLayout(new FillLayout(SWT.HORIZONTAL));
		purchaseorderEdit = new PurchaseOrderEdit(editorComposite, SWT.NONE, labelWidth);
		purchaseorderEdit.addPropertyChangeListener("do-edit",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				...
				if(e.getNewValue().toString().equals("UNIT-SELECTED")) {
					setSelected((PurchaseOrderModel) dataTable.getSelection()[0].getData());
				}
				dataTable.setRedraw(true);
				dataViewer.refresh();
			}
		});
		purchaseorderEdit.addPropertyChangeListener("add-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				addRecord();
			}
		});
		purchaseorderEdit.addPropertyChangeListener("delete-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				deleteRecord();
			}
		});
		GridLayout gridLayout = (GridLayout) purchaseorderEdit.getLayout();
			...
			protected IStatus run(final IProgressMonitor monitor) {
				...
				Display.getDefault().syncExec(new Runnable() {
					...
					public void run() {
						...
						monitor.worked(3);
						dataViewer.setInput(new WritableList(data.getFilterData(), PurchaseOrderModel.class));
						monitor.worked(4);
					}
				});
				monitor.worked(5);
...
//						prime();
	...
	public void selectDetail(int headerSequence) {
		...
		try {
			data.toPurchaseOrderModelList(" WHERE UnitSequence = " + headerSequence);
		} catch (Exception e) {
		e.printStackTrace();
		}
		dataViewer.setInput(new WritableList(data.getData(), PurchaseOrderModel.class));
...
//						prime();
	...
	public PurchaseOrderModel getSelected() {
		return selected;
	}
	private void deleteRecord() {
		...
		for(Object object:selection.toArray()) {
			PurchaseOrderModel purchaseorderModel = (PurchaseOrderModel) object;
			if(connectionSet.size()==0) {
				try {
					connectionSet.add(new PurchaseOrderTransaction(purchaseorderModel));
			...
			if(selection.size()>0) {
				try {
					data.doDelete((PurchaseOrderModel) selection.getFirstElement(), filtered);
				} catch (Exception e) {
					MessageDialog.openError(parent.getShell(), "Edit Error",e.getMessage());
					return;
				}
			...
			if(selection.size()==0) {
				if(dataTable.getItemCount()>0) {
					...
					purchaseorderEdit.destroyEditors();
					try {
						purchaseorderEdit.createEditors(new PurchaseOrderModel(true));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		try {
			...
			if(meta.IS_ELEMENT) {
				...
				for(Object selected:addSelection) {
					...
					newPurchaseOrder.setUnitSequence(getHeaderSequence());
			int next = data.nextUnitSequence();
			newPurchaseOrder.setUnitSequence(next);
					Method getProperty;
					Method setProperty;
					for(String columnName:meta.ELEMENT_SELECTOR.getReturnSet()) {
						...
						setProperty = newPurchaseOrder.getClass().getDeclaredMethod("set" + columnName,String.class);
						setProperty.setAccessible(true);
						setProperty.invoke(newPurchaseOrder, getProperty.invoke(selected));
					}
					if(connectionSet.size()==0) {
						connectionSet.add(new PurchaseOrderTransaction(newPurchaseOrder));
						connectionSet.iterator().next().begin();
					}
					connectionSet.iterator().next().insert(newPurchaseOrder);
					boolean filtered = false;
					if(data.getFilterData().size()!=0) {
						if(data.getFilterData().size()!=data.getData().size()) {
							filtered = true;
					...
					data.getData().add(newPurchaseOrder);
					dataViewer.setInput(null);
					if(filtered) {
						data.getFilterData().add(newPurchaseOrder);
						dataViewer.setInput(new WritableList(data.getFilterData(), PurchaseOrderModel.class));
					} else {
						dataViewer.setInput(new WritableList(data.getData(), PurchaseOrderModel.class));
					}
	...
	public void setSelected(PurchaseOrderModel selected) {
		boolean error = false;
		if(selected==null) {
			return;
		}
		...
		try {
			if(!error) {
				purchaseorderEdit.createEditors(selected);
			}
		} catch (Exception e) {
	...
	public void switchTransaction(PurchaseOrderModel unit) throws Exception {
		if(connectionSet.size()>0) {
			connectionSet.iterator().next().commit();
			connectionSet.iterator().next().disconnect();
			connectionSet.remove(connectionSet.iterator().next());
		}
		connectionSet.add(new PurchaseOrderTransaction(unit));
		try {
			connectionSet.iterator().next().begin();
		} catch (Exception e) {
			connectionSet.iterator().next().disconnect();
