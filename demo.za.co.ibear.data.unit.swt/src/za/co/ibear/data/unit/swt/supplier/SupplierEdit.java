...
public class SupplierEdit extends Composite {
	...
	private CTabItem supplierTab;
	private Composite supplierComposite;
	private Text search;
	private TableViewer viewer;
	private Table table;
	private Supplier meta = new Supplier();
	private List<UnitEditor> editorData = null;
	private SupplierModel editModel = null;
	...
	int labelWidth = 200;
	public SupplierEdit(Composite parent, int style, int labelWidth) throws Exception {
		super(parent, style);
		...
		supplierTab = new CTabItem(folder, SWT.NONE);
		supplierTab.setText("Supplier");
		supplierComposite = new Composite(folder, SWT.NONE);
		supplierTab.setControl(supplierComposite);
		GridLayout gl_supplierComposite = new GridLayout(3, false);
		gl_supplierComposite.verticalSpacing = 3;
		gl_supplierComposite.marginWidth = 0;
		gl_supplierComposite.marginHeight = 0;
		gl_supplierComposite.horizontalSpacing = 3;
		supplierComposite.setLayout(gl_supplierComposite);
		search = new Text(supplierComposite, SWT.SEARCH | SWT.ICON_SEARCH | SWT.CANCEL | SWT.BORDER);
		...
		Button add = new Button(supplierComposite, SWT.NONE);
		...
		Button delete = new Button(supplierComposite, SWT.NONE);
		if(meta.IS_READ_ONLY) {
		...
		viewer = new TableViewer(supplierComposite, SWT.BORDER);
		...
		createEditors(null);
		final SupplierEditFilter rowFilter = new SupplierEditFilter();
	...
	public void createEditors(SupplierModel data) throws Exception {
		if (data != null) {
			this.editModel = data;
		} else {
	...
	public SupplierModel getEditModel() {
		return editModel;
	}
	public void setEditModel(SupplierModel editModel) {
...
class SupplierEditFilter extends ViewerFilter {
...
class SupplierEditComparator extends ViewerComparator {
	private int propertyIndex;
	private static final int DESCENDING = 1;
	private int direction = DESCENDING;
	public SupplierEditComparator() {
