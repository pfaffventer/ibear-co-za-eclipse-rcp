...
public class PurchaseOrderEdit extends Composite {
	...
	private CTabItem purchaseorderTab;
	private Composite purchaseorderComposite;
	private Text search;
	private TableViewer viewer;
	private Table table;
	private PurchaseOrder meta = new PurchaseOrder();
	private List<UnitEditor> editorData = null;
	private PurchaseOrderModel editModel = null;
	...
	int labelWidth = 200;
	public PurchaseOrderEdit(Composite parent, int style, int labelWidth) throws Exception {
		super(parent, style);
		...
		purchaseorderTab = new CTabItem(folder, SWT.NONE);
		purchaseorderTab.setText("PurchaseOrder");
		purchaseorderComposite = new Composite(folder, SWT.NONE);
		purchaseorderTab.setControl(purchaseorderComposite);
		GridLayout gl_purchaseorderComposite = new GridLayout(3, false);
		gl_purchaseorderComposite.verticalSpacing = 3;
		gl_purchaseorderComposite.marginWidth = 0;
		gl_purchaseorderComposite.marginHeight = 0;
		gl_purchaseorderComposite.horizontalSpacing = 3;
		purchaseorderComposite.setLayout(gl_purchaseorderComposite);
		search = new Text(purchaseorderComposite, SWT.SEARCH | SWT.ICON_SEARCH | SWT.CANCEL | SWT.BORDER);
		...
		Button add = new Button(purchaseorderComposite, SWT.NONE);
		...
		Button delete = new Button(purchaseorderComposite, SWT.NONE);
		if(meta.IS_READ_ONLY) {
		...
		viewer = new TableViewer(purchaseorderComposite, SWT.BORDER);
		...
		createEditors(null);
		final PurchaseOrderEditFilter rowFilter = new PurchaseOrderEditFilter();
	...
	public void createEditors(PurchaseOrderModel data) throws Exception {
		if (data != null) {
			this.editModel = data;
		} else {
	...
	public PurchaseOrderModel getEditModel() {
		return editModel;
	}
	public void setEditModel(PurchaseOrderModel editModel) {
...
class PurchaseOrderEditFilter extends ViewerFilter {
...
class PurchaseOrderEditComparator extends ViewerComparator {
	private int propertyIndex;
	private static final int DESCENDING = 1;
	private int direction = DESCENDING;
	public PurchaseOrderEditComparator() {
