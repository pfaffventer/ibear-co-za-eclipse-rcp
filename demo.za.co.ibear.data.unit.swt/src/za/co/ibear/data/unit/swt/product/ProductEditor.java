...
import za.co.ibear.data.unit.product.ProductLink;
import za.co.ibear.data.unit.product.ProductModel;
import za.co.ibear.data.unit.product.ProductTransaction;
...
public class ProductEditor extends UnitEditComposite implements IProgressMonitor {
	...
	private Product meta = new Product();
	protected boolean SHELL_PARENT = false;
	private Set<ProductTransaction> connectionSet = new HashSet<ProductTransaction>();
	...
	private Text search;
	private ProductComparator comparator = null;
	private String queryColumn = null;
	private List<String> queryItems = null;
	
	public ProductEditor(final Composite parent, int style, int[] editorWeights,int labelWidth) throws Exception  {
		super(parent, style);
		this.parent = parent;
		...
		data = new ProductLink();
		if(!meta.IS_ELEMENT) {
			data.toProductModelList(null);
		}
		...
		for(String bcombo_meta:meta.BCOMBO) {
			...
			if(type==BComboConstant.QUERY) {
				queryColumn = bcombo;
				String queryName = bcombo_meta.split(">>")[2];
				String queryDescription = bcombo_meta.split(">>")[3];
				filterItem = new TableItem(filterTable, SWT.NONE);
				editor = new TableEditor(filterTable);
				editor.grabHorizontal = true;
				editor.horizontalAlignment = SWT.FILL;
				filter = new UnitBrowseCombo(filterTable,1850,queryDescription,queryName,false,null,true);
				filter.addPropertyChangeListener("selection_changed",new PropertyChangeListener() {
					public void propertyChange(final PropertyChangeEvent e) {
						queryItems = new ArrayList<String>();
						@SuppressWarnings("unchecked")
						List<TableItem> selection = (List<TableItem>) e.getNewValue();
						for(TableItem item:selection) {
							queryItems.add(((StockQueryModel) item.getData()).getProduct());
						}
					}
				});
				filter.addPropertyChangeListener("do-refresh",new PropertyChangeListener() {
					public void propertyChange(final PropertyChangeEvent e) {
						query();
					}
				});
		...
		dataViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				setSelected((ProductModel) selection.getFirstElement());
			}
		});
		...
		dataViewer.setInput(new WritableList(data.getData(), ProductModel.class));
		editorComposite = new Composite(sash, SWT.NONE);
		editorComposite.setLayout(new FillLayout(SWT.HORIZONTAL));
		productEdit = new ProductEdit(editorComposite, SWT.NONE, labelWidth);
		productEdit.addPropertyChangeListener("do-edit",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				...
				if(e.getNewValue().toString().equals("UNIT-SELECTED")) {
					setSelected((ProductModel) dataTable.getSelection()[0].getData());
				}
				dataTable.setRedraw(true);
				dataViewer.refresh();
			}
		});
		productEdit.addPropertyChangeListener("add-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				addRecord();
			}
		});
		productEdit.addPropertyChangeListener("delete-record",new PropertyChangeListener() {
			public void propertyChange(final PropertyChangeEvent e) {
				deleteRecord();
			}
		});
		GridLayout gridLayout = (GridLayout) productEdit.getLayout();
			...
			protected IStatus run(final IProgressMonitor monitor) {
				...
				Display.getDefault().syncExec(new Runnable() {
					...
					public void run() {
						...
						monitor.worked(3);
						dataViewer.setInput(new WritableList(data.getFilterData(), ProductModel.class));
						monitor.worked(4);
					}
				});
				monitor.worked(5);
...
//						prime();
	...
	public void selectDetail(int headerSequence) {
		...
		try {
			data.toProductModelList(" WHERE UnitSequence = " + headerSequence);
		} catch (Exception e) {
		e.printStackTrace();
		}
		dataViewer.setInput(new WritableList(data.getData(), ProductModel.class));
	...
	private void query() {
		Job job = new Job("JOB_QUERY") {
			@Override
			protected IStatus run(final IProgressMonitor monitor) {
				monitor.beginTask("Executing query.",5);
				monitor.worked(2);
				Display.getDefault().syncExec(new Runnable() {
					@Override
					public void run() {
						data.doQuery(queryColumn,queryItems);
						monitor.worked(3);
						dataViewer.setInput(new WritableList(data.getFilterData(), ProductModel.class));
						monitor.worked(4);
					}
			...
			public void done(IJobChangeEvent event) {
				...
				if (!event.getResult().isOK()) {
					Display.getDefault().syncExec(new Runnable() {
						@Override
						public void run() {
							MessageDialog.openError(parent.getShell(), "Filter Error", "Fatal error could not run query.");
						}
					});
				} else {
					for(BCombo bcombo:unitSet) {
...
//						prime();
	...
	private void deleteRecord() {
		...
		for(Object object:selection.toArray()) {
			...
			if(connectionSet.size()==0) {
				try {
					connectionSet.add(new ProductTransaction(productModel));
			...
			if(selection.size()>0) {
				try {
					data.doDelete((ProductModel) selection.getFirstElement(), filtered);
				} catch (Exception e) {
					MessageDialog.openError(parent.getShell(), "Edit Error",e.getMessage());
					return;
				}
			...
			if(selection.size()==0) {
				if(dataTable.getItemCount()>0) {
					...
					productEdit.destroyEditors();
					try {
						productEdit.createEditors(new ProductModel(true));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		try {
			...
			if(meta.IS_ELEMENT) {
				...
				for(Object selected:addSelection) {
					...
					newProduct.setUnitSequence(getHeaderSequence());
			int next = data.nextUnitSequence();
			newProduct.setUnitSequence(next);
					Method getProperty;
					Method setProperty;
					for(String columnName:meta.ELEMENT_SELECTOR.getReturnSet()) {
						...
						setProperty = newProduct.getClass().getDeclaredMethod("set" + columnName,String.class);
						setProperty.setAccessible(true);
						setProperty.invoke(newProduct, getProperty.invoke(selected));
					}
					if(connectionSet.size()==0) {
						connectionSet.add(new ProductTransaction(newProduct));
						connectionSet.iterator().next().begin();
					}
					connectionSet.iterator().next().insert(newProduct);
					boolean filtered = false;
					if(data.getFilterData().size()!=0) {
						if(data.getFilterData().size()!=data.getData().size()) {
							filtered = true;
					...
					data.getData().add(newProduct);
					dataViewer.setInput(null);
					if(filtered) {
						data.getFilterData().add(newProduct);
						dataViewer.setInput(new WritableList(data.getFilterData(), ProductModel.class));
					} else {
						dataViewer.setInput(new WritableList(data.getData(), ProductModel.class));
					}
	...
	public void setSelected(ProductModel selected) {
		boolean error = false;
		if(selected==null) {
			return;
		}
		...
		try {
			if(!error) {
				productEdit.createEditors(selected);
			}
		} catch (Exception e) {
	...
	public void switchTransaction(ProductModel unit) throws Exception {
		if(connectionSet.size()>0) {
			connectionSet.iterator().next().commit();
			connectionSet.iterator().next().disconnect();
			connectionSet.remove(connectionSet.iterator().next());
		}
		connectionSet.add(new ProductTransaction(unit));
		try {
			connectionSet.iterator().next().begin();
		} catch (Exception e) {
			connectionSet.iterator().next().disconnect();
